...
class P2PTradingPolicyAuction(TorchRLModule, ValueFunctionAPI):
    ...
    def setup(self):
        ...
        self.time_blocks: int = self.model_config.get("time_blocks")
        self.ts_layer = nn.LSTM(
            input_size=3,
            hidden_size=self.model_config.get("common_hidden_size"),
            num_layers=2,
            batch_first=True,
        )
        # self.generation_layer = nn.LSTM(
        ...
        decoder_input_size = (
            self.model_config.get("common_hidden_size")
            # + self.model_config.get("demand_hidden_size")
            # + self.model_config.get("generation_hidden_size")
            # + self.model_config.get("market_price_hidden_size")
            + 64
    ...
    def _compute_embeddings(self, batch):
        ...
        #     self.generation_layer, obs["generation"]
        # )
        ts_embeds = self._return_last_timestep(self.ts_layer, ts_values)
        embeds = torch.cat(
            (
                scaler_embeds,
                # demand_embeds,
...
class P2PTradingPolicyAuctionV2(TorchRLModule, ValueFunctionAPI):
    """
    Custom RLModule for the P2P energy trading environment.
    ...
    def _compute_embeddings(self, batch):
        ...
        scaler_embeds = self.encoder_layer(scalers)
        ts_embeds = self.conv_block(ts_values)
        embeds = torch.cat(
            (
                scaler_embeds,
                ts_embeds,
    ...
    def _forward_train(self, batch, **kwargs):
        ...
        q_mean_raw = self.q_head(embeddings)  # (B, 24)
        p_mean_raw = self.p_head(embeddings)  # (B, 24)
